...
export const useCommunityHooks = () => {
  ...
  const getCommunityPromptList = async (selectedCategory, sortOrder, searchQuery, currentPage) => {
    ...
    try {
      ...
      const response = await sendRequest(communityIntstance, "get", "", { params });
  ...
  //ToDo - login 구분이 되면 위에 api와 합치기

  // const getCommunityPreviewPromptList = async (selectedCategory, sortOrder) => {
  //   try {
  //     const params = {
  //       category:selectedCategory,
  //       search: "",
  ...
  //     }
  //
  //     const response = await sendRequest(communityIntstance, "get", "/preview", { params });
  //
  //     console.log("api호출 성공");
  //     setCommunityPromptList(response.data.selectPrompt);
  //     setCommunityPromptListPage(response.data.pageInfo);
  //   } catch (error) {
  //     setCommunityPromptList([]);
  //     console.error("Error:", error);
  //   }
