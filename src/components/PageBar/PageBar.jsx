...
function PageBar() {
    const [pageState, setPageState] = useRecoilState(communityPromptListPageState);
    ...
    const startPage = Math.max(1, userCurrentPage - Math.floor(maxPageButtons / 2));
                ...
                <div>
                    {isLoading ? (
                        ...
                        <div className={styles.pagination}>
                            <button onClick={() => handlePageChange(userCurrentPage - 1)} disabled={userCurrentPage === 1}>&lt;</button>
                            {pageNumbers.map(number => (
                                <button
                                    key={number}
                                    onClick={() => handlePageChange(number)}
                                    className={userCurrentPage === number ? styles.active : ""}
                                >
                                    {number}
                                </button>
                            ))}
                            <button onClick={() => handlePageChange(userCurrentPage + 1)} disabled={userCurrentPage === totalPages}>&gt;</button>
                        </div>
                    )}
                </div>
            )}
